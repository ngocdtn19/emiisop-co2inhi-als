...
def interpolate_to_new_pressure(temp, hcho, old_p, new_p):
    """
    Interpolate temperature and hcho to new pressure levels for each point.
    """
    # Interpolate temperature
    fv = "extrapolate"
    interp_func_temp = interp1d(old_p, temp, axis=-1, bounds_error=False, fill_value=fv)
    new_temp = interp_func_temp(new_p)

    # Interpolate hcho
    interp_func_hcho = interp1d(old_p, hcho, axis=-1, bounds_error=False, fill_value=fv)
...
def ak_apply_chaser(sat_ds, chaser_t, chaser_ch2o, chaser_ps, sat_pressure, case):
    ...
    ds = xr.Dataset(
        {
            "T": (("time", "lat", "lon", "layer"), Ctemp_filtered["T"].values),
            "CH2O": (("time", "lat", "lon", "layer"), Cch2o_filtered["CH2O"].values),
            "PS": (("time", "lat", "lon", "layer"), Cps_filtered["PS"].values),
        },
        coords={
            "time": Ctemp_filtered.time.values,
            "lat": Ctemp_filtered.lat.values,
            "lon": Ctemp_filtered.lon.values,
            "layer": np.arange(len(Ctemp_filtered.layer.values)),
        },
    )
    sat_intep_temp, sat_interp_hcho = xr.apply_ufunc(
        interpolate_to_new_pressure,
        ds["T"],
        ds["CH2O"],
        ds["PS"],
        sat_ps_filtered["layer_pressure"],
    ...
    sat_levels = sat_ps_filtered["layer_pressure"].values.shape[-1]
    ...
    sat_ps_df = sat_ps_filtered.mean(dims).to_dataframe()
...
def ak_apply_to_chaser_do():
    ...
    # P = ctm_sigma_a + surface_pressure*ctm_sigma_b
    # P_to_h1= (-1)*((np.log(np.array(P1)/101325))*8.314*np.array(T))/(0.028*9.8);
    # P_to_h2= (-1)*((np.log(np.array(P2)/101325))*8.314*np.array(T2))/(0.028*9.8);
    # H= abs(P_to_h2 - P_to_h1)
    # Partial_col=((np.array(HCHO)*1e-9*H*P1*1e-4)/(1.38e-23*np.array(T).astype(float)))*AK
    ...
    for case in CASES:
        ...
        ps_ds = xr.open_dataset(f"{chaser_case_dir}/ps.nc").resample(time="M").mean()

        t_ds = prep_chaser_for_ak(t_ds)
        ch2o_ds = prep_chaser_for_ak(ch2o_ds)
        ps_ds = prep_chaser_for_ak(ps_ds)

        t_ds["time"] = t_ds.time.values.astype("datetime64[M]").astype("datetime64[ns]")
        ...
        ps_ds = ps_ds.rename({"hcho": "PS"})

        ch2o_tropo_ak = ak_apply_chaser(
            ds_tropo, t_ds, ch2o_ds, ps_ds, tropo_pressure, f"{case}_TROPO"
        )
        ch2o_omi_ak = ak_apply_chaser(
            ds_omi, t_ds, ch2o_ds, ps_ds, omi_pressure, f"{case}_OMI"
        )

        out_dir_case = f"{out_dir}/{case}"
